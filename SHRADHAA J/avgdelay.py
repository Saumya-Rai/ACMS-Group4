...
flightList = data['callsign'].unique()
# print(type(flightList))
flightAverageDelay = pd.DataFrame(columns=['callsign', 'average delay'])
for flight in flightList:
    
    flight_df = data.loc[data['callsign'] == flight]
    #print flight_df['callsign']
	
    #print(flight_df['flight number'], flight_df['delay'])
    avr = np.mean(flight_df['delay'].tolist())
    #callsign = flight_df['callsign'][1]
    #average in minutes to be stored
    if "+" in str(flight):
        modified="6E" + str(flight).split("+")[1]
        flightAverageDelay.loc[len(flightAverageDelay)] = [flight, avr/60] 
    else:
        flightAverageDelay.loc[len(flightAverageDelay)] = [flight, avr/60] 
print(flightAverageDelay)

flightAverageDelay.to_csv('Flight Average Delay.csv',  sep=',', index=False)